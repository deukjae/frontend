/*
    자바스크립트 객체 : 원시값(number,string 등) 을 제외한 함수,배열도 객체 object입니다.
                    : 데이터 타입확인연산자 typeof
...
//          <--------- 오라클에서 select 한 결과를 자바객체로 매핑해서 구현


// OPEN API(REST API): 포탈서비스, 공공기관, 단체, 학교... 데이터를 공개하고 제공
//              date.go.kr 에서 json 문자열(경량데이터)을 제공하는 사례를 찾아보세요.

//              개발자  -----------  data.go.kr 공개 데이터 --> 서버
//                         <-------- JSON 문자열을 전달 <------ (REST 서비스)
